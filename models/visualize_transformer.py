...
class Transformer(dg.Layer):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        memory, encoder_attn_weights = self.encoder(src, src_mask=mask, pos=pos_embed) # [bs, h * w, c]
        hs, decoder_attn_weights = self.decoder(tgt, memory, memory_mask=mask, pos=pos_embed, query_pos=query_embed)
...
class TransformerEncoder(dg.Layer):
    ...
    def forward(self, src, src_mask=None, pos=None):
        ...
        for layer in self.layers:
            output, self_attn_weights = layer(output, src_mask=src_mask, pos=pos)
            attention_weight.append(self_attn_weights)
...
class TransformerDecoder(dg.Layer):
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, 
        ...
        attention_weight = []
        for layer in self.layers:
            output, self_attn_weights, multihead_attn_weights = layer(output, memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
                           pos=pos, query_pos=query_pos)
            
            attention_weight.append((self_attn_weights, multihead_attn_weights))
...
class TransformerEncoderLayer(dg.Layer):
    def __init__(self, d_model, nhead, dim_feedforward, dropout=0.1,
        ...
        self.self_attn = MultiHeadAttention(d_model, nhead, dropout=dropout,
                                            weight_attr=weight_attrs[0], bias_attr=bias_attrs[0], need_weights=True)
    ...
    def forward_post(self, src, src_mask=None, pos=None):
        ...
        src2, self_attn_weights = self.self_attn(q, k, src, src_mask)
...
class TransformerDecoderLayer(dg.Layer):
    ...
    def __init__(self, d_model, nhead, dim_feedforward, dropout=0.1,
        ...
        self.multihead_attn = MultiHeadAttention(d_model, nhead, dropout=dropout,
                                             weight_attr=weight_attrs[1], bias_attr=bias_attrs[1], need_weights=True)
        self.linear1 = Linear(d_model, dim_feedforward, 
    ...
    def forward_post(self, tgt, memory, tgt_mask=None, memory_mask=None, pos=None, query_pos=None):
        ...
        tgt2, self_attn_weights = self.self_attn(q, k, tgt, tgt_mask)
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)

        q = self.with_pos_embed(tgt, query_pos)
        k = self.with_pos_embed(memory, pos)
        tgt2, multihead_attn_weights = self.multihead_attn(q, k, memory, memory_mask)
